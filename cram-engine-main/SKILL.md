...
| `/cram <课程名> retry <知识点>` | 对指定知识点重新执行阶段2-3-4微型流程：重讲（四步教学）→ 重测（2道新题，至少一道题型不同于原题）→ 补漏（答错则诊断+换讲法+重测+顽固判定）。结果按知识点名称更新进度文件中的对应记录（保留历史在折叠区块中） |
| `/cram <课程名> stubborn` | 列出所有顽固点，逐个执行微型闭环（重讲→重测→补漏），专门用于第二天集中攻克顽固点 |
...
⑤ 列出要考的知识点，每行一个。
  ...
  ⚠️ 如果用户未输入任何知识点就回车跳过，提示：
  "至少需要一个知识点才能开始。请列出要考的知识点，每行一个。"
  不允许空列表进入下一步。重复此步骤直到用户提供至少一个知识点。
...
- 用户说"减速"→ 每个点多配一个例子
- 用户说"跳过"→ 停止阶段2，直接进入阶段3。跳过的知识点（阶段2未标记 [x] 的点）在阶段3中不纳入出题范围，可通过 /cram retry 或阶段4补漏单独处理
...
3. **出题前过滤**（不用模型，由你直接控制）：
   - 读取进度文件，仅对阶段2中已标记为 [x] 的知识点出题
   - 阶段2被跳过的知识点（未标记 [x]）不纳入阶段3出题范围
4. **出题策略**（不用模型，由你直接控制）：
...
4. must_know 答错的考点：额外多出3道新题，确保至少两轮全对才算过关
...
当执行 `/cram <课程名> resume` 时，必须先校验进度文件结构：

1. 读取进度文件 `~/.cram-engine/progress/<课程名>-progress.md`
2. 检查必需段落是否存在：`## 状态`、`## 阶段1完成`、`## 阶段2进度`、`## 阶段3`、`## 阶段4`、`## 顽固点`
3. 检查 `## 阶段2进度` 中的任务列表格式是否正确（每行必须以 `- [x]` 或 `- [ ]` 开头）
4. 如果格式异常（缺少必需段落或任务列表格式错误），输出提示：
   "⚠️ 进度文件格式异常，可能存在以下问题：<列出具体问题>。是否尝试自动修复？"
   等待用户确认后再继续。
